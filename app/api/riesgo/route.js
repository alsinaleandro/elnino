  ...
  for (const feature of geojsonData.features) {
    ...
    if (geometry.type === 'Polygon') {
      if (isPointInPolygon(point, geometry.coordinates)) {
        return { zone: name, color: { 'Zona Prohibida': '#2ec4b6', 'Zona de Restricción Severa': '#1d4ed8', 'Zona de Restricción Severa Temporaria': '#f59e0b', 'Zona de Restricción Leve': '#7dd3a8' }[name] || '#4f7ee3' }
      }
    }
...
export async function GET(request) {
  ...
  if (Number.isNaN(latitude) || Number.isNaN(longitude)) {
    return Response.json({ error: 'Parámetros lat y lng requeridos y válidos.' }, { status: 400 })
  }

  const geojsonPath = path.join(process.cwd(), 'client', 'public', 'riesgo_hidrico_AMGR_todas.geojson')
  const raw = fs.readFileSync(geojsonPath, 'utf8')
  const geojsonData = JSON.parse(raw)
  const result = findRiskZoneForPoint(latitude, longitude, geojsonData)
