...
const riskMeta = {
  'Zona Prohibida': { label: 'Muy alto', severity: 100, description: 'Se recomienda evitar la zona por restricciones importantes.' },
  'Zona de Restricción Severa': { label: 'Alto', severity: 75, description: 'Existe una restricción fuerte para actividades sensibles.' },
  'Zona de Restricción Severa Temporaria': { label: 'Medio alto', severity: 60, description: 'La zona presenta una restricción temporal importante.' },
  'Zona de Restricción Leve': { label: 'Bajo', severity: 30, description: 'La zona tiene menor nivel de restricción, pero requiere atención.' },
  'Fuera de cualquier zona de riesgo': { label: 'Sin riesgo', severity: 0, description: 'La ubicación no coincide con ninguna zona de riesgo del mapa.' },
  'Sin datos de riesgo': { label: 'Sin datos', severity: 0, description: 'Todavía no se pudo identificar la zona de riesgo asociada.' },
}
...
function getFeatureColor(feature) {
  ...
  return properties.color || properties.fill || properties.stroke || riskColors[category] || '#4f7ee3'
}
...
function isPointInPolygon(point, polygonCoordinates) {
  ...
  for (const ring of rings) {
    if (isPointInRing(point, ring)) return true
  }
  return false
}
...
function findRiskZoneForPoint(latitude, longitude, geojsonData) {
  if (!geojsonData || !geojsonData.features) {
    return { name: 'Sin datos de riesgo', color: '#4f7ee3' }
  }
  ...
  for (const feature of geojsonData.features) {
    ...
    if (geometry.type === 'Polygon') {
      if (isPointInPolygon(point, geometry.coordinates)) {
        return { name, color: riskColors[name] || '#4f7ee3' }
      }
    }
  ...
  return { name: 'Fuera de cualquier zona de riesgo', color: '#2e7d32' }
}
...
export default function Page() {
  ...
  const updateRiskFromPosition = async (latitude, longitude) => {
    const apiBaseUrl = typeof window !== 'undefined' ? window.location.origin : ''

    try {
      const response = await fetch(`${apiBaseUrl}/api/riesgo?lat=${encodeURIComponent(latitude)}&lng=${encodeURIComponent(longitude)}`, { cache: 'no-store' })
  ...
  const centerOnLocation = () => {
    ...
    map.flyTo([coords[1], coords[0]], 16, { animate: true, duration: 1 })
  }
  ...
  const determineRiskZone = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        await updateRiskFromPosition(latitude, longitude)
  ...
  const loadRiverInfo = async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/parana`, { cache: 'no-store' })
      if (!response.ok) throw new Error('No se pudo consultar la fuente del río Paraná')
      const data = await response.json()
      setRiverInfo(data)
    } catch (error) {
      setRiverInfo({ error: 'No se pudo consultar la información del río Paraná en la fuente oficial.' })
    } finally {
    ...
    const loadLeaflet = async () => {
      ...
      fetch('/riesgo_hidrico_AMGR_todas.geojson')
        ...
        .then((data) => {
          ...
          if (coords && locationReady) updateRiskFromPosition(coords[1], coords[0])
        })
        .catch(() => {
          setStatus('No se pudieron cargar las capas de riesgo hídrico.')
        })

      const resizeMap = () => setTimeout(() => map.invalidateSize(), 0)
      resizeMap()
  ...
  return (
    <main className="app-shell">
      <nav className="tab-bar" aria-label="Pestañas de la app">
        <button type="button" className={activeTab === 'map' ? 'tab-button active' : 'tab-button'} onClick={() => setActiveTab('map')}>Mapa</button>
        <button type="button" className={activeTab === 'risk' ? 'tab-button active' : 'tab-button'} onClick={() => setActiveTab('risk')}>Riesgo</button>
      </nav>
      ...
      <div className={activeTab === 'map' ? 'panel visible' : 'panel hidden'}>
        ...
        <div className="map-wrapper">
          ...
          <div className="map-actions">
            {locationReady && (
              <button type="button" className="locate-button secondary" onClick={centerOnLocation}>Centrar</button>
            )}
          </div>
        </div>
        ...
        <section className="risk-panel">
          ...
          <div className="risk-meter" aria-label="Indicador de nivel de riesgo">
            <div className="risk-meter-track">
              <div className="risk-meter-fill" style={{ width: `${currentRisk.severity}%`, background: riskColor }} />
            </div>
          ...
          <div className="risk-details">
            ...
            <div>
              ...
              <strong>{coords ? `${coords[1].toFixed(5)}, ${coords[0].toFixed(5)}` : 'Sin ubicación'}</strong>
            </div>
          </div>
          ...
          <div className="river-info">
            ...
            {!riverInfo && !riverLoading && (
              <button type="button" className="locate-button" onClick={loadRiverInfo}>Cargar datos</button>
            )}
            ...
            {riverLoading ? (
              ...
              <>
                <p>{riverInfo.error}</p>
                <button type="button" className="locate-button secondary" onClick={loadRiverInfo}>Reintentar</button>
              </>
            ) : riverInfo ? (
              <>
